...
build_ssg_paths() {
    SSG_HOME="/ssg"
    PARTITION_ROOT="${SSG_HOME}/etc/conf/partitions"
    PARTITION_NAMES=`ls ${PARTITION_ROOT} | grep -v template_ `
}
...
get_partition_name() {
	...
	echo -n "Please specify the partition you wish to configure [default_]: "
        read PARTITION_NAME
        if [ -z ${PARTITION_NAME} ] ; then
            echo "A partition name was not specified, using the default_ partition"
            PARTITION_NAME="default_"
            isValid="y"
        else
            if [ -d "${PARTITION_ROOT}/${PARTITION_NAME}" ] ; then
                isValid="y"
            else
                echo
                echo "${PARTITION_NAME} is not an available partition"
                echo "please enter one of the available partitions"
                echo
                isValid="n"
            fi
        fi
...
set_keystore_paths() {
    ...
    KEYSTORE_PROPERTIES_FILE="${PARTITION_ROOT}/${PARTITION_NAME}/keystore.properties"
    KEYSTORE_DIR="${PARTITION_ROOT}/${PARTITION_NAME}/keys"

    WAR_FILE="$TOMCAT_HOME/webapps/ROOT.war"
    SERVER_XML_FILE="${PARTITION_ROOT}/${PARTITION_NAME}/server.xml"
...
get_setkeys_type() {
    ...
    echo "-----------------------------"
    echo -n "Please choose an option [1]: "
    read MENU_CHOICE
...
get_hostname() {
    ...
    echo -n "Please provide host name [$dflt]: "
    read HOST_NAME
    if [ -e $HOST_NAME ]; then
        HOST_NAME=$dflt
    fi
...
get_password() {
    ...
    then
        ...
        echo -n "Please choose a keystore password: "
	...
	echo -n "Please repeat: "
        ...
        echo -n "Please type in the existing keystore password: "
        read -s KEYSTORE_PASSWORD
    fi
}

do_keystore_gen_and_config() {
    ...
    perl -pi.bak - "${SERVER_XML_FILE}" <<EOM
...
do_setkeys_interview() {
    ...
    get_password
    do_keystore_gen_and_config
    chown ssgconfig:gateway "${PARTITION_ROOT}/${PARTITION_NAME}/keystore.properties"
    chown ssgconfig:gateway "${PARTITION_ROOT}/${PARTITION_NAME}/server.xml"
    chown -R ssgconfig:gateway "${PARTITION_ROOT}/${PARTITION_NAME}/keys/"
    chmod -f 0644 "${PARTITION_ROOT}/${PARTITION_NAME}/keystore.properties"
    chmod -f 0644 "${PARTITION_ROOT}/${PARTITION_NAME}/server.xml"
    chmod -f 0755 "${PARTITION_ROOT}/${PARTITION_NAME}/keys/"
    chmod -f 0644 ${PARTITION_ROOT}/${PARTITION_NAME}/keys/*
    echo
    echo "Done".
}
