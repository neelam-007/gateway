...
use constant KMPFILE_INITIAL_CONTENTS => <<'EOM';
# Generated file; do not edit, as it may be deleted or regenerated by ncipherconfig
config.profile=ncipher.sworld.rsa.oaep
EOM
...
sub doGeneateNewKmp() {
    unlink(KMPFILE);
    die "Unable to recreate " . KMPFILE . " as it already exists and cannot be deleted: $!\n"
        if (-e KMPFILE);

    eval {
        my $fh = new IO::File();
        $fh->open(">" . KMPFILE) and do {
            print $fh KMPFILE_INITIAL_CONTENTS;
            undef $fh;
        };
    ...
    if ($@) {
        # Ensure we never leave a kmp.properties file behind if kmp configuration fails
        unlink(KMPFILE);
        die $@;
    }
}
