...
CREATE TABLE jms_endpoint(
  ...
  security_zone_goid binary(16),
  is_passthrough_message_rules tinyint NOT NULL default '1',
  primary key(goid),
...
CREATE TABLE audit_detail (
  ...
  PRIMARY KEY (goid),
  KEY idx_component_id (component_id),
  KEY idx_audit_goid (audit_goid),
  FOREIGN KEY (audit_goid) REFERENCES audit_main (goid) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARACTER SET utf8;

DROP TABLE IF EXISTS audit_detail_params;
CREATE TABLE audit_detail_params (
  audit_detail_goid binary(16) NOT NULL,
  position integer NOT NULL,
  value MEDIUMTEXT NOT NULL,
  PRIMARY KEY (audit_detail_goid, position),
  FOREIGN KEY (audit_detail_goid) REFERENCES audit_detail (goid) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARACTER SET utf8;
...
INSERT INTO rbac_role (goid, version, name, tag, entity_type, entity_goid, description, user_created) VALUES (toGoid(0, -650),0,'Manage Message Destinations', null,null,null, 'Users assigned to the {0} role have the ability to read, create, update and delete message destinations.',0);
