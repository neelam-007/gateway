...
public class SecurityContextTokenHandler {
     ...
     * @throws com.l7tech.util.TooManyChildElementsException if there is more than one SOAP header, security header,
    ...
    public byte[] getSessionIdFromWSCToken(Document soapMsg) throws IOException, TooManyChildElementsException {
        ...
        Element idel = DomUtils.findOnlyOneChildElementByName(token, WSC_NAMESPACE, SCTOKEN_ID_ELNAME);
        String childText = DomUtils.findFirstChildTextNode(idel);
     ...
     * @throws com.l7tech.util.TooManyChildElementsException if there was more than one SOAP header, security header,
    ...
    public Element getSecurityContextTokenElement(Document doc) throws TooManyChildElementsException {
        ...
        Element header = DomUtils.findOnlyOneChildElementByName(doc.getDocumentElement(), soapns, SoapConstants.HEADER_EL_NAME);
        if (header == null)
           return null;
        Element security = DomUtils.findOnlyOneChildElementByName(header, SoapConstants.SECURITY_NAMESPACE, SoapConstants.SECURITY_EL_NAME);
        if (security == null)
            security = DomUtils.findOnlyOneChildElementByName(header, SoapConstants.SECURITY_NAMESPACE2, SoapConstants.SECURITY_EL_NAME);
        if (security == null)
            return null;
        Element token = DomUtils.findOnlyOneChildElementByName(security, WSC_NAMESPACE, SCTOKEN_ELNAME);
        return token;
    }
     ...
     * @throws com.l7tech.util.TooManyChildElementsException if there is more than one MessageNumber element in this
    ...
    public Long getMessageNumber(Element securityContextToken) throws TooManyChildElementsException {
        if (securityContextToken == null)
            return null;
        Element mn = DomUtils.findOnlyOneChildElementByName(securityContextToken, L7_NAMESPACE, MESSAGE_NUMBER_ELNAME);
        if (mn == null)
            return null;
        try {
            return Long.valueOf(DomUtils.findFirstChildTextNode(mn));
    ...
    private void setMessageNumber(Element securityContextToken, Long messageNumber) {
        ...
        DomUtils.removeChildElementsByName(securityContextToken, L7_NAMESPACE, MESSAGE_NUMBER_ELNAME);
        ...
        Text messageNumberText = DomUtils.createTextNode(doc, String.valueOf(messageNumber));
    ...
    private void appendCreationElement(Element securityCtxTokenEl, long creationTimeStamp) {
        ...
        Text valNode = DomUtils.createTextNode(securityCtxTokenEl, stamp);
    ...
    private void appendIDElement(Element securityCtxTokenEl, byte[] sessionid) {
        ...
        Text valNode = DomUtils.createTextNode(securityCtxTokenEl, sessionIdToURI(sessionid));
