...
public class SamlAssertionFaultDetail implements SoapFaultDetail {
    ...
    public SamlAssertionFaultDetail(String assertionId, String faultString, String faultCode, String reasonCode) {
        ...
        Element badAss = DomUtils.createAndAppendElementNS(detail, SecureSpanConstants.FAULTDETAIL_SAML_ASSERTIONID, ns, pf);
        badAss.appendChild(DomUtils.createTextNode(badAss, assertionId));
        if (reasonCode != null) {
            Element reason = DomUtils.createAndAppendElementNS(detail, SecureSpanConstants.FAULTDETAIL_SAML_REASON,  ns, pf);
            reason.appendChild(DomUtils.createTextNode(reason, reasonCode));
        }
        faultDetail = detail;
    }
