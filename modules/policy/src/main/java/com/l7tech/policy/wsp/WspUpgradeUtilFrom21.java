    ...
    */
    ...
    static Element translateOldXmlSecurityElement(Element in, WspVisitor visitor) throws InvalidPolicyStreamException {
        ...
        try {
            ...
            Element elements = DomUtils.findOnlyOneChildElementByName(in, in.getNamespaceURI(), "Elements");
            List items = elements == null ? Collections.EMPTY_LIST :
                    DomUtils.findChildElementsByName(elements, elements.getNamespaceURI(), "item");
            for (Iterator i = items.iterator(); i.hasNext();) {
                Element item = (Element)i.next();
                // Ignore Cipher and KeyLength -- was always AES128 in 2.1
                Element encryptionEl = DomUtils.findOnlyOneChildElementByName(item, item.getNamespaceURI(), "Encryption");
                boolean encryption = encryptionEl != null && "true".equalsIgnoreCase(encryptionEl.getAttribute("booleanValue").trim());
                Element preconditionXpath = DomUtils.findOnlyOneChildElementByName(item, item.getNamespaceURI(), "PreconditionXpath");
                Element elementXpath = DomUtils.findOnlyOneChildElementByName(item, item.getNamespaceURI(), "ElementXpath");
            ...
            if (seenPreconditions.size() > 0) {
                ...
                String expr = TextUtils.join(new StringBuffer("not(("), ")|(",
