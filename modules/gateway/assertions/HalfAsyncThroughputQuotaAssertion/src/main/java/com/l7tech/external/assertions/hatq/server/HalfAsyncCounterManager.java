 ...
 *
 */
public class HalfAsyncCounterManager extends HibernateDaoSupport {
    private final Logger logger = Logger.getLogger(HalfAsyncCounterManager.class.getName());
    ...
    public HalfAsyncCounterManager(PlatformTransactionManager transactionManager) {
    ...
    private void updateCounter(Connection conn, String counterName, Counter newValues) throws SQLException {
    ...
    public long incrementAsyncAndReturnValue(final String counterName, final long timestamp, final int fieldOfInterest) {
    ...
    public long checkValueAndRequestAsyncIncrementIfWithinLimit(final String counterName,
    ...
    private void scheduleAsyncCounterStep(String counterName, CounterStep inc) {
        ...
        if (!enqueued)
            workQueue.queue.add(inc); // might block here if updates have fallen way behind
    }
                    ...
                    public Void doInHibernate(Session session) throws HibernateException, SQLException {
                        session.doWork(new Work() {
                            ...
                            public void execute(Connection connection) throws SQLException {
                                ...
                                if (updated) {
                                    updateCounter(connection, counterName, counter);
                                }
                            }
                        });
                        return null;
    ...
    private static class WorkQueue {
        ...
        private final Queue<CounterStep> queue = new LinkedBlockingQueue<CounterStep>(counterQueueSize);
