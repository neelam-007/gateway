...
public class ApiKeyDataResourceHandlerTest {
    private ApiKeyDataResourceHandler handler;
    private ApiKeyResource resource;
    @Mock
    private PortalGenericEntityManager<ApiKeyData> manager;
    @Mock
    private ResourceTransformer<ApiKeyResource, ApiKeyData> transformer;

    @Before
    public void setup() {
        handler = new ApiKeyDataResourceHandler(manager, transformer);
    ...
    public void getNotFound() throws Exception {
        ...
        assertNull(result);
        verify(manager).find("k1");
        verify(transformer, never()).entityToResource(Matchers.<ApiKeyData>any());
    }
    ...
    public void putUpdate() throws Exception {
        final ApiKeyData entity = createDefaultEntity();
        when(transformer.resourceToEntity(resource)).thenReturn(entity);
        when(manager.find("k1")).thenReturn(new ApiKeyData());
    ...
    public void putAddOrUpdateFindException() throws Exception {
        ...
        try {
            ...
            verify(transformer).resourceToEntity(resource);
            verify(manager).find("k1");
            verify(manager, never()).add(Matchers.<ApiKeyData>any());
            verify(manager, never()).update(Matchers.<ApiKeyData>any());
            verify(transformer, never()).entityToResource(Matchers.<ApiKeyData>any());
            throw e;
        }
        fail("Expected FindException");
