    ...
    public CommandResult execute(ProducerCommandContext<?> context) throws Exception {
        if (context instanceof CloseCommandContext) {
            ...
            log("deleteEntity",
                    "entitySetName", c.getEntitySetName(),
                    "entityKey", c.getEntityKey());
        } else if (context instanceof GetEntitiesCountCommandContext) {
            GetEntitiesCountCommandContext c = (GetEntitiesCountCommandContext) context;
            log("getEntities",
                    "entitySetName", c.getEntitySetName(),
                    "queryInfo", c.getQueryInfo());
        } else if (context instanceof UpdateEntityCommandContext) {
            UpdateEntityCommandContext c = (UpdateEntityCommandContext) context;
            log("createEntity",
                    "entitySetName", c.getEntitySetName(),
                    "entity", c.getEntity());
        } else {
