...
import static com.l7tech.external.assertions.apiportalintegration.server.AbstractPortalGenericEntityManager.*;
...
public class PolicyHelper {
  ...
  PolicyHelper(@NotNull final ApplicationContext context) {
    this(context.getBean("policyManager", PolicyManager.class), context.getBean("policyVersionManager", PolicyVersionManager.class), context.getBean("transactionManager", PlatformTransactionManager.class), context.getBean("licenseManager", AssertionLicense.class), context.getBean("serverPolicyValidator", PolicyValidator.class));
  }

  PolicyHelper(@NotNull final PolicyManager policyManager, @NotNull final PolicyVersionManager policyVersionManager, @NotNull final PlatformTransactionManager transactionManager, @NotNull final AssertionLicense licenseManager, @NotNull final PolicyValidator policyValidator) {
  ...
  private Assertion getPolicyAssertionChild(final Policy policy, final int position) {
    Assertion child = null;

    if (policy != null) {
      try {
        final Assertion assertion = policy.getAssertion();
        final CompositeAssertion compositeAssertion = assertion instanceof CompositeAssertion ? (CompositeAssertion) assertion : null;
  ...
  static class OperationResult {
    ...
    public boolean hasError() {
      if ((result != null && result.length() > 0) || (policyValidationResult != null && policyValidationResult.getStatus() == PolicyValidationResult.ValidationStatus.ERROR)) {
        return true;
      }
      return false;
    }
