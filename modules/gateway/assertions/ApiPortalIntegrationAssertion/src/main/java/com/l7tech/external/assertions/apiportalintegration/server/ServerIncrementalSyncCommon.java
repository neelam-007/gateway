...
public class ServerIncrementalSyncCommon {
    ...
    public final static String getSyncDeletedEntities(String type) {
        return String.format("SELECT ENTITY_UUID FROM DELETED_ENTITY WHERE TYPE = '%s' AND DELETED_TS > ? AND DELETED_TS <= ?", type);
    }

    public final static String getSyncUpdatedAppEntities(List<String> columns) {
        String columnStr = Joiner.on(",").join(columns);
        return String.format(
                "SELECT %s FROM APPLICATION a  \n" +
                        ...
                        "WHERE a.API_KEY IS NOT NULL AND a.STATUS IN ('ENABLED','DISABLED') AND ( (a.MODIFY_TS > ? and a.MODIFY_TS <=  ? ) OR (a.MODIFY_TS =0 and a.CREATE_TS > ? and  a.CREATE_TS <=  ?) OR ( o.MODIFY_TS > ? and  o.MODIFY_TS <=  ?) OR (t.TENANT_GATEWAY_UUID = ? AND t.SYNC_LOG IS NOT NULL))", columnStr
        );
    }
