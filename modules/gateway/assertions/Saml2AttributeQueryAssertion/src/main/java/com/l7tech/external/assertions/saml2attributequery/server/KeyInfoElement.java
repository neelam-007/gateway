 ...
 * see {@link com.l7tech.security.xml.KeyInfoDetails}.
 */
public class KeyInfoElement implements ParsedElement {
     ...
     * @throws org.xml.sax.SAXException if the format of this KeyInfo is invalid or not supported
     ...
     * @throws com.l7tech.util.InvalidDocumentFormatException   If we can't figure out the KeyInfo format.
     * @throws com.l7tech.security.xml.UnexpectedKeyInfoException       If we understood the KeyInfo, but it is not referring to our certificate.
     * @throws java.security.cert.CertificateException             If we need the encoded form of the certificate but it is invalid.
     */
            ...
            throws InvalidDocumentFormatException, CertificateException
    {
        ...
        if (str == null) {
            ...
            if (embedded != null) {
                ...
                Element x509DataEle = DomUtils.findOnlyOneChildElementByName(keyInfo, SoapConstants.DIGSIG_URI, "X509Data");
                if(x509DataEle != null) {
                    Element x509SubjectName = DomUtils.findOnlyOneChildElementByName(x509DataEle, SoapConstants.DIGSIG_URI, "X509SubjectName");
                    if(x509SubjectName != null) {
                        // Fix up the subject DN so that it can be matched
                        String subjectName = x509SubjectName.getTextContent().toLowerCase();
                        StringBuilder sb = new StringBuilder();
                        int start = 0;
                        boolean inQuotes = false;
                        for(int i = 0;i < subjectName.length();i++) {
                            if(!inQuotes && subjectName.charAt(i) == ',' || i == subjectName.length() - 1) {
                                String part = subjectName.substring(start, i < subjectName.length() - 1 ? i : i + 1).trim();
                                start = i + 1;
                                ...
                                if(part.matches("[^=]+=\"[^\"]*\"")) {
                                    ...
                                    part = part.substring(0, equalsPos + 1) + part.substring(equalsPos + 2, part.length() - 1);
                                }
                                sb.append(part);
                            } else if(!inQuotes && subjectName.charAt(i) == '"') {
                                inQuotes = true;
                            } else if(inQuotes && subjectName.charAt(i) == '"') {
                                inQuotes = false;
                            }
                        ...
                        SignerInfo found = securityTokenResolver.lookupPrivateKeyByKeyName(subjectName);
                        if(found != null) {
                            return found;
                        }
                    }
