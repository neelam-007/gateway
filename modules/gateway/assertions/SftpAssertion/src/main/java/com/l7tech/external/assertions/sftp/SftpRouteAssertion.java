 ...
 * 
 */
public class SftpRouteAssertion extends RoutingAssertion implements UsesVariables, SetsVariables {

    /** Timeout (in milliseconds) when opening SFTP Connection. */
    ...
    /** SFTP server host name. Can contain context variables. */
    private String hostName;

    /** Port number. Can contain context variables. */
    private String port = Integer.toString(DEFAULT_SFTP_PORT);
    ...
    private final static String baseName = "Route via SFTP";
    ...
    // Metadata
    //
    private static final String META_INITIALIZED = SftpRouteAssertion.class.getName() + ".metadataInitialized";
    protected static final Logger logger = Logger.getLogger(SftpRouteAssertion.class.getName());
    ...
    public AssertionMetadata meta() {
        ...
        meta.put(AssertionMetadata.LONG_NAME, "Route requests from the Gateway to another server, using SFTP. This assertion supports the SSH2 protocol for SFTP transfer.");
        ...
        meta.put(AssertionMetadata.PROPERTIES_EDITOR_CLASSNAME, "com.l7tech.external.assertions.sftp.console.SftpRouteAssertionPropertiesPanel");
        meta.put(AssertionMetadata.PROPERTIES_ACTION_NAME, "SFTP Route Assertion Properties");
        meta.put(AssertionMetadata.POLICY_NODE_NAME_FACTORY, new AssertionNodeNameFactory<SftpRouteAssertion>() {
            @Override
            public String getAssertionName(final SftpRouteAssertion assertion, final boolean decorate) {
                if(!decorate) return baseName;

                final StringBuilder sb = new StringBuilder("Send via SFTP to Server ");
                sb.append(assertion.getHost());
                return AssertionUtils.decorateName(assertion, sb.toString());
            }
        });
        meta.put(WSP_EXTERNAL_NAME, "SftpRouteAssertion");
        ...
        // that is, we want our required feature set to be "assertion:SftpRouting" rather than "set:modularAssertions"
