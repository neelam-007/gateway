...
public class ServerSiteMinderAuthenticateAssertionTest {
    ...
    public void shouldNotAuthenticateUserWhenSetLastCredentialAndOnlyJWT() throws Exception {
        ...
        // when Use Last credential and jwt is set, SiteMinderCredentials should be empty
        verify(mockHla, times(1)).processAuthenticationRequest(eq(new SiteMinderCredentials()), isNull(String.class), isNull(String.class), eq(mockContext), eq(true));
    ...
    public void shouldNotAuthenticateWhenX509CertificateAndJWTSpecified() throws Exception {
        ...
        // when both x509 and jwt is set, SiteMinderCredentials are overwritten by either jwt or x509.
        verify(mockHla, times(1)).processAuthenticationRequest(eq(new SiteMinderCredentials(JWT_HMAC)), isNull(String.class), isNull(String.class), eq(mockContext), eq(true));
    }
    ...
    public void shouldAuthenticateWhenJWTSpecified() throws Exception {
        ...
        when(mockHla.processAuthenticationRequest(eq(new SiteMinderCredentials(JWT_HMAC)), anyString(), isNull(String.class), any(SiteMinderContext.class), eq(true))).thenReturn(1);
