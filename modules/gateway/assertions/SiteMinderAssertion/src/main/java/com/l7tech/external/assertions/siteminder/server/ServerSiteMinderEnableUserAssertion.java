...
/**
 * Server side implementation of the SiteMinderEnableUserAssertion.
 *
 * @see com.l7tech.external.assertions.siteminder.SiteMinderEnableUserAssertion
 */
public class ServerSiteMinderEnableUserAssertion extends AbstractServerSiteMinderAssertion<SiteMinderEnableUserAssertion> {
    private final SecurePasswordManager securePasswordManager;
    private final String[] variablesUsed;

    public ServerSiteMinderEnableUserAssertion(final SiteMinderEnableUserAssertion assertion, final ApplicationContext context) throws PolicyAssertionException {
    ...
    public AssertionStatus doCheckRequest(final PolicyEnforcementContext context,
        ...
        try {
            ...
            final int result = hla.processEnableUserRequest(adminUsername, adminPassword, domOid, username, smContext);
            if (result == 0) {
                logAndAudit(AssertionMessages.SINGLE_SIGN_ON_FINE, (String) (assertion.meta().get(AssertionMetadata.SHORT_NAME)),
                        "Enabled user account for username: '" + username + "'");
                return AssertionStatus.NONE;
            } else {
                context.setVariable(SiteMinderEnableUserAssertion.REASON_CODE_CONTEXT_VAR_NAME, result);
                logAndAudit(AssertionMessages.SINGLE_SIGN_ON_WARNING, (String) (assertion.meta().get(AssertionMetadata.SHORT_NAME)),
                        "Failed to enable user account for username: '" + username + "'. Reason code: " + result);
                return AssertionStatus.FALSIFIED;
            }
