...
/**
 * <p> Copyright (C) 2011 Layer 7 Technologies Inc.</p>
 * <p> @author wlui </p>
 * $Id$
 */

public class CopyFederatedIdentityProviderAction extends NewProviderAction {
    static final Logger log = Logger.getLogger(CopyFederatedIdentityProviderAction.class.getName());
    ...
    public CopyFederatedIdentityProviderAction(IdentityProviderNode node) {
        super(node);
        this.fipConfig = null;
    }
    ...
    protected void performAction() {
        ...
        SwingUtilities.invokeLater(new Runnable() {
            @Override
            public void run() {
                fipConfig = new FederatedIdentityProviderConfig(getIdentityProviderConfig((EntityHeaderNode) node));
        ...
        public void entityAdded(final EntityEvent ev) {
            SwingUtilities.invokeLater(new Runnable() {
                ...
                public void run() {
                    ...
                    IdentitiesRootNode rootNode  = tree.getRootNode();
                    if (tree.hasBeenExpanded(new TreePath(rootNode.getPath()))) {
                        DefaultTreeModel model = (DefaultTreeModel)tree.getModel();

                        final AbstractTreeNode newChildNode = TreeNodeFactory.asTreeNode(eh, null);
                        model.insertNodeInto(newChildNode, rootNode, rootNode.getInsertPosition(newChildNode));
