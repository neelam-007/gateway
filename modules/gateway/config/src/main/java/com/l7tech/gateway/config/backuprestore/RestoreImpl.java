...
final class RestoreImpl implements Restore{
    ...
    static class RestoreException extends Exception{
    ...
    public Result restoreComponentOS(boolean isRequired, boolean verbose, PrintStream printStream) throws RestoreException {
    ...
    public Result restoreComponentAudits(final boolean isRequired, final boolean verbose, final PrintStream printStream)
        ...
        if(this.dbRestorer== null) {
            ...
            if (auditsFile != null && image.getImageVersion() == BackupImage.ImageVersion.AFTER_FIVE_O &&
                ...
                ImportExportUtilities.logAndPrintMessage(logger, Level.WARNING, msg, verbose, this.printStream);
            } else {
        ...
        try {
            ...
            ImportExportUtilities.logAndPrintMessage(logger, Level.INFO, msg, verbose, this.printStream, false);
            dbRestorer.restoreAudits();
            msg = ". Done";
            ImportExportUtilities.logAndPrintMessage(logger, Level.INFO, msg, verbose, this.printStream);
    ...
    public Result restoreComponentMainDb(final boolean isRequired,
        ...
        if(!wasNewDbCreated){
            ...
            checkAndThrowIfSSGRunning(verbose, printStream);
            try {
                preMigrateLicense = dbRestorer.getExistingLicense();
            } catch (SQLException e) {
                String msg = "Could not check for existing license: " + e.getMessage();
                ImportExportUtilities.logAndPrintMessage(logger, Level.SEVERE, msg, verbose, this.printStream);
            ...
            if (mappingPath != null && !wasNewDbCreated) {
                try {
                    ImportExportUtilities.logAndPrintMessage(logger, Level.INFO,
                            "loading mapping file " + mappingPath, verbose, this.printStream);
                ...
                ImportExportUtilities.logAndPrintMessage(logger, Level.INFO,
                        "Mapping file '" + mappingPath+"' ignored as new database was created", verbose, this.printStream);
            }
        }
        ...
        if (image.getImageVersion() == BackupImage.ImageVersion.AFTER_FIVE_O) {
            ...
            if(!myCnf.exists() || !myCnf.isFile()){
                ...
                String etcFolder = SyspropUtil.getString("com.l7tech.config.restore.mycnfdir", "/etc");
