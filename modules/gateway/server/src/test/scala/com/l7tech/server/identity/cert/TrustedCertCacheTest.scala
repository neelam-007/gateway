...
class TrustedCertCacheTest extends SpecificationWithJUnit {
  "Should have immutable results" in {
    ...
    Some(testCache.findByPrimaryKey(1)).foreach(_.setIssuerDn("CN=Test")) must throwA[IllegalStateException]
  ...
  def createTestCache: TrustedCertCache = {
    new TrustedCertCacheImpl( new TestTrustedCertManager(
      trustedCert( 1, TestDocuments.getWssInteropAliceCert ),
      trustedCert( 2, TestDocuments.getWssInteropBobCert ),
      trustedCert( 3, TestDocuments.getWssInteropIpCert, Seq(TrustedFor.SIGNING_CLIENT_CERTS) )
    ) )
  }

  def trustedCert( oid: Long, certificate: X509Certificate, trustedFor: Seq[TrustedFor] = Nil ): TrustedCert = {
    val trustedCert = new TrustedCert();
    trustedCert.setOid( oid )
