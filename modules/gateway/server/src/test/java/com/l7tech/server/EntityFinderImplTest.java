...
public class EntityFinderImplTest {
    ...
    public void setup() {
        ...
        finder.setGenericEntityManager(genericEntityManager);
    ...
    @Test
    public void findGenericEntityDelegatesToManager() throws Exception {
        final Goid pk = new Goid(0, 1);
        final GenericEntity genericEntity = new GenericEntity();
        genericEntity.setGoid(pk);
        when(genericEntityManager.findByPrimaryKey(pk)).thenReturn(genericEntity);
        final GenericEntity found = finder.find(GenericEntity.class, pk);
        assertEquals(genericEntity, found);
        verify(genericEntityManager).findByPrimaryKey(pk);
    }

    @Test
    public void findGenericEntityNonGoidPrimaryKey() throws Exception {
        final GenericEntity genericEntity = new GenericEntity();
        when(genericEntityManager.findByPrimaryKey(any(Goid.class))).thenReturn(genericEntity);
        final GenericEntity found = finder.find(GenericEntity.class, 1234L);
        assertEquals(genericEntity, found);
        verify(genericEntityManager).findByPrimaryKey(any(Goid.class));
    }
