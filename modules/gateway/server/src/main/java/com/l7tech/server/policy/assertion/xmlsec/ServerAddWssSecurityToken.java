...
public class ServerAddWssSecurityToken extends AbstractMessageTargetableServerAssertion<AddWssSecurityToken> {
            ...
            throws IOException, PolicyAssertionException
    {
        ...
        if (SecurityTokenType.WSS_USERNAME == tokenType) {
            ...
            logAndAudit(AssertionMessages.ADD_WSS_TOKEN_UNSUPPORTED_TYPE, assertion.getTokenType().getName());
    ...
    private AssertionStatus addEncryptedKey(PolicyEnforcementContext context, Message message, String messageDescription, AuthenticationContext authContext, DecorationRequirements dreq) throws IOException, PolicyAssertionException {
        ...
        try {
            ...
            logAndAudit(AssertionMessages.ADD_WSS_TOKEN_MULTIPLE_REQ_TOKENS);
    ...
    private AssertionStatus addSamlAssertion(PolicyEnforcementContext context, Message message, String messageDescription, AuthenticationContext authContext, DecorationRequirements dreq) {
        ...
        if (assertionVar == null || assertionVar.length() < 1) {
            logAndAudit(AssertionMessages.ASSERTION_MISCONFIGURED, "no SAML assertion variable was provided");
            return AssertionStatus.SERVER_ERROR;
        }

        Element samlElement;
        try {
            final Map<String,Object> vars = context.getVariableMap( variableNames, getAudit() );
            Object samlVal = ExpandVariables.processSingleVariableAsObject( Syntax.getVariableExpression(assertionVar), vars, getAudit() );

            if (null == samlVal) {
                logAndAudit(AssertionMessages.ADD_WSS_TOKEN_NOT_SAML, "SAML variable not found or null");
                return AssertionStatus.SERVER_ERROR;
            }
            ...
            if (samlVal instanceof Element) {
                ...
                logAndAudit(AssertionMessages.ADD_WSS_TOKEN_NOT_SAML, "SAML variable is unsupported type: " + samlVal.getClass().getName());
                return AssertionStatus.SERVER_ERROR;
            }

        } catch (SAXException e) {
            //noinspection ThrowableResultOfMethodCallIgnored
            logAndAudit(AssertionMessages.ADD_WSS_TOKEN_NOT_SAML, new String[] {ExceptionUtils.getMessage(e)}, ExceptionUtils.getDebugException(e));
            return AssertionStatus.SERVER_ERROR;
        }
        ...
        if (samlAssertion.hasSubjectConfirmationEncryptedKey()) {
            ...
            try {
                ...
                logAndAudit(AssertionMessages.ADD_WSS_TOKEN_SAML_SECRET_KEY_UNAVAILABLE, null, e);
            } catch (UnexpectedKeyInfoException e) {
                logAndAudit(AssertionMessages.ADD_WSS_TOKEN_SAML_SECRET_KEY_UNAVAILABLE_WITH_MORE_INFO, ExceptionUtils.getMessage(e));
    ...
    private AssertionStatus addWsscContext(PolicyEnforcementContext context, Message message, String messageDescription, AuthenticationContext authContext, DecorationRequirements dreq) {
        ...
        if (wsscVarName == null || wsscVarName.length() < 1) {
            logAndAudit(AssertionMessages.ASSERTION_MISCONFIGURED, "no secure conversation session variable name was provided");
            return AssertionStatus.SERVER_ERROR;
        }

        final Map<String,Object> vars = context.getVariableMap( variableNames, getAudit() );
        final Object wsscVal = ExpandVariables.processSingleVariableAsObject( Syntax.getVariableExpression(wsscVarName), vars, getAudit() );
        if ( wsscVal == null ) {
            logAndAudit(AssertionMessages.NO_SUCH_VARIABLE_WARNING, wsscVarName);
            return AssertionStatus.SERVER_ERROR;
        }

        if (!(wsscVal instanceof SecureConversationSession)) {
            logAndAudit(AssertionMessages.ADD_WSS_TOKEN_NOT_SESSION);
            return AssertionStatus.SERVER_ERROR;
        }
    ...
    private AssertionStatus applyUsernameTokenGatheredCredentialsDecorationRequirements(PolicyEnforcementContext context, Message message, String messageDescription, AuthenticationContext authContext)
        ...
        AddWssSignatureSupport.SignedElementSelector signedElementSelector = new AddWssSignatureSupport.SignedElementSelector() {
                    ...
                    throws PolicyAssertionException
            {
                ...
                if (name == null) {
                    logAndAudit(AssertionMessages.ADD_WSS_TOKEN_NO_USERNAME);
                    return -1;
                }

                if (assertion.isIncludePassword()) {
                    if (pass == null) {
                        logAndAudit(AssertionMessages.ADD_WSS_TOKEN_NO_PASSWORD);
                        return -1;
                    }
                } else {
                    pass = null;
