...
public class ServerRequestSwAAssertion extends AbstractServerAssertion<RequestSwAAssertion> {
    ...
    private static final long KB_TO_B_MULT = 1024L;
    ...
    public AssertionStatus checkRequest(final PolicyEnforcementContext context) throws IOException, PolicyAssertionException {
        try {
            if (!context.getRequest().isSoap()) {
                logAndAudit( AssertionMessages.SWA_NOT_SOAP );
                return AssertionStatus.NOT_APPLICABLE;
            }
        } catch (SAXException e) {
            logAndAudit( AssertionMessages.SWA_INVALID_XML, ExceptionUtils.getMessage( e ) );
            return AssertionStatus.BAD_REQUEST;
        }

        if (!context.getRequest().getMimeKnob().isMultipart()) {
            logAndAudit( AssertionMessages.SWA_NOT_MULTIPART );
        ...
        try {
            ...
            logAndAudit( AssertionMessages.EXCEPTION_WARNING_WITH_MORE_INFO, new String[]{ "Error retrieving xml document from request" }, e );
            ...
            logAndAudit( AssertionMessages.EXCEPTION_INFO_WITH_MORE_INFO, new String[]{ "The required attachment " + e.getWhatWasMissing() + "was not found in the request" }, e );
            return AssertionStatus.FALSIFIED;
    ...
    private void processAttachments(final PolicyEnforcementContext context,
        ...
        for (String bindingName : (Set<String>) bindings.keySet()) {
            ...
            for (String boName : binding.getBindingOperations().keySet()) {
                ...
                if (result == null || result.size() == 0) {
                    logAndAudit( AssertionMessages.SWA_OPERATION_NOT_FOUND, bo.getXpath() );
                    continue;
                }

                if (result.size() > 1) {
                    logAndAudit( AssertionMessages.SWA_REPEATED_OPERATION, bo.getXpath() );
                ...
                int type = (int) operationNodeRequest.getNodeType();

                if (type != (int) Node.ELEMENT_NODE ) {
                    logAndAudit( AssertionMessages.SWA_OPERATION_NOT_ELEMENT_NODE, bo.getXpath(), operationNodeRequest.toString() );
        ...
        logAndAudit( AssertionMessages.SWA_INVALID_OPERATION );
    ...
    private void processAttachmentsForOperation(final PolicyEnforcementContext context,
            ...
            throws JaxenException, IOException, NoSuchPartException, SAXException, AttachmentProcessingException {
        logAndAudit( AssertionMessages.SWA_OPERATION_FOUND, bo.getName() );
        ...
        for (String parameterName : (Set<String>) bo.getMultipart().keySet()) {
            ...
            if (result.size() > 1) {
                logAndAudit( AssertionMessages.SWA_REPEATED_MIME_PART, bo.getXpath(), part.getName() );
            ...
            int type = (int) parameterNodeRequest.getNodeType();

            if (type != (int) Node.ELEMENT_NODE ) {
                logAndAudit( AssertionMessages.SWA_PARAMETER_NOT_ELEMENT_NODE, bo.getXpath(), part.getName(), parameterNodeRequest.toString() );
                throw new AttachmentProcessingException("Unexpected result from Xpath for part " + part.getName(), AssertionStatus.FAILED);
            }

            logAndAudit( AssertionMessages.SWA_PARAMETER_FOUND, part.getName() );
            ...
            if (hrefs.isEmpty()) {
                logAndAudit( AssertionMessages.SWA_REFERENCE_NOT_FOUND, part.getName() );
    ...
    private int processAttachmentsForPart(final PolicyEnforcementContext context,
        ...
        for (Attr href : hrefs) {
            ...
            logAndAudit( AssertionMessages.SWA_REFERENCE_FOUND, part.getName(), mimePartCIDUrl );
            int cpos = mimePartCIDUrl.indexOf(":");
            if (cpos < 0) {
                logAndAudit( AssertionMessages.SWA_INVALID_CONTENT_ID_URL, mimePartCIDUrl );
            ...
            if (mimepartRequest != null) {
                ...
                if ( part.isRequireSignature() ) {
                    if ( wssResults == null || !partSigned(mimepartRequest, wssResults.getPartsThatWereSigned())) {
                        logAndAudit( AssertionMessages.SWA_NOT_SIGNED, part.getName(), mimePartCIDUrl );
                ...
                if (!part.validateContentType(mimepartRequest.getContentType())) {
                    if (part.getContentTypes().length > 1) {
                        logAndAudit( AssertionMessages.SWA_NOT_IN_CONTENT_TYPES, mimePartCIDUrl, part.retrieveAllContentTypes() );
                    } else {
                        logAndAudit( AssertionMessages.SWA_BAD_CONTENT_TYPE, mimePartCIDUrl, part.retrieveAllContentTypes() );
                    }
                    throw new AttachmentProcessingException("Validation failed for part " + part.getName(), AssertionStatus.FALSIFIED);
                }

                totalLen += (int) mimepartRequest.getActualContentLength();

                // check the max. length allowed
                if ( (long) totalLen > (long) part.getMaxLength() * KB_TO_B_MULT) {
                    if (hrefs.size() > 1) {
                        logAndAudit( AssertionMessages.SWA_TOTAL_LENGTH_LIMIT_EXCEEDED, part.getName(), String.valueOf( hrefs.size() ), String.valueOf( part.getMaxLength() ) );
                    } else {
                        logAndAudit( AssertionMessages.SWA_PART_LENGTH_LIMIT_EXCEEDED, mimePartCIDUrl, String.valueOf( part.getMaxLength() ) );
                    }
                ...
                logAndAudit( AssertionMessages.SWA_NO_ATTACHMENT, mimePartCIDUrl );
            ...
            if (!attachment.isValidated()) {
                ...
                if (constraints != null) {
                    if (validExtraAttachment(attachment, constraints)) {
                        logAndAudit( AssertionMessages.SWA_EXTRA_ATTACHMENT, attachmentName );
                    ...
                    logAndAudit( AssertionMessages.SWA_EXTRA_ATTACHMENT_DROPPED, attachmentName );
                    dropAttachments.add(attachment);
                }
                else {
                    logAndAudit( AssertionMessages.SWA_UNEXPECTED_ATTACHMENT, attachmentName );
    ...
    private Map<String,long[]> getPermittedExtrasForBindingOperation(final BindingOperationInfo bo) {
        ...
        for(Map.Entry<String,MimePartInfo> entry : ((Map<String,MimePartInfo>)xtraMPI).entrySet()) {
            ...
            long maxLength = (long) mpi.getMaxLength() * KB_TO_B_MULT;
    ...
    private void removeAttachmentHrefs(Element element, String id) {
        ...
        for (int n=0; n<children.getLength(); n++) {
            ...
            if ( (int) child.getNodeType() == (int) Node.ELEMENT_NODE ) {
    ...
    private boolean validExtraAttachment(PartInfo attachment, long[] constraints) throws NoSuchPartException, IOException {
        ...
        if (constraints[0] >= 0L ) {
           ...
           logAndAudit( AssertionMessages.SWA_EXTRA_LENGTH_EXCEEDED, Long.toString( configuredMaxLn ) );
        }
