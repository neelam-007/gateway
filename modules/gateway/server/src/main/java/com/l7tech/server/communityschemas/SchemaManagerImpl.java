...
/**
 * Creates and manages all in-memory instances of {@link CompiledSchema}.
 * <p/>
 * Users of {@link CompiledSchema}s should feel free to retain {@link SchemaHandle}s for any that are
 * actually in use, but should not cache them any longer than needed;
 */
...
public class SchemaManagerImpl implements PostStartupApplicationListener, SchemaManager, SchemaSourceResolver.SchemaInvalidationListener {
    ...
    /**
     * Stores the latest version of each registered schema string (policy assertion
    ...
    /**
     * Add this schema to the TNS cache.
     * <p/>
     * Caller must hold the write lock.
     *
     * @param newSchema the schema.
     */
    private void addNewSchemaToCache(final CompiledSchema newSchema) {
        addNewSchemaToCache( newSchema, new HashSet<CompiledSchema>() );
    }
    ...
    private void addNewSchemaToCache( final CompiledSchema newSchema,
        ...
        for (final SchemaHandle child : newSchema.getDependencies().values())
            addNewSchemaToCache(child.getTarget(), processed);
    ...
    public Set<String> getRequiredSchemaUris() {
        ...
        // this covers the use case of those
            ...
            throws SAXException, IOException
    {
        ...
        addNewSchemaToCache(schemaHandle.getTarget());
        return schemaHandle;
    }
        ...
        try {
            ...
            final CompiledSchema newSchema =
                    new CompiledSchema(tns, systemId, schemaDoc, source.getResolverId(), mangledElement, softwareSchema, this,
                            directImports, directIncludes, source.isTransient());
    ...
    /**
     * Removes a CompiledSchema from caches.  
     * <p/>
