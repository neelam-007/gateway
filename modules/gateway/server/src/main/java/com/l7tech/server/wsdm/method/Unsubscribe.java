...
public class Unsubscribe extends ESMMethod {
    private Element unSubscribeEl;
    private String subscriptionIdValue;

    private Unsubscribe(String subscriptionId, Element unSubscribeEl) throws GenericNotificationExceptionFault {
        this.unSubscribeEl = unSubscribeEl;
    ...
    public static Unsubscribe resolve(Document doc) throws GenericNotificationExceptionFault {
        try {
            Element bodychild = getFirstBodyChild(doc);
            if ( bodychild != null ) {
                if (testElementLocalName(bodychild, "Unsubscribe")) {
                    return new Unsubscribe(getSubscriptionIdAddressingParameter(doc), bodychild);
                }
            }
