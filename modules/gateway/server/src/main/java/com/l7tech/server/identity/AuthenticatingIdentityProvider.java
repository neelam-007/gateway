...
public interface AuthenticatingIdentityProvider<UT extends User, GT extends Group, UMT extends UserManager<UT>, GMT extends GroupManager<UT, GT>> extends IdentityProvider<UT, GT, UMT , GMT> {
    ...
    /**
     * Same as {@link AuthenticatingIdentityProvider#authenticate(LoginCredentials)} but allows the user to be
     * upgraded for password storage if required.
     * @param allowUserUpgrade if true user can be ugpraded if needed. Should not be true for message traffic (currently).
     */
    AuthenticationResult authenticate( LoginCredentials pc, boolean allowUserUpgrade) throws AuthenticationException;
