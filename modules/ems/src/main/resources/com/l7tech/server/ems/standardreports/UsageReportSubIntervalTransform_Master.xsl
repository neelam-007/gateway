...
<xsl:transform version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="returnValue[@toVariable='TOTAL']">
        <xsl:copy>
            <xsl:apply-templates select="node()|@*"/>
        </xsl:copy>
        <xsl:text>
    ...
    <xsl:template match="/jasperReport/detail/band/subreport/reportElement">
        <xsl:element name="reportElement">
            <xsl:choose>
                <xsl:when test="$useDynamicWidths = 1" >
                    <xsl:attribute name="width">
                        <xsl:value-of select="$RuntimeDoc/JasperRuntimeTransformation/subReportWidth" />
                    </xsl:attribute>
