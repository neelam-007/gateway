...
for /D %%i in ("%SSG_HOME%\etc\conf\partitions\*") do if not "%%~ni"=="partitiontemplate_" call EditSsgLog.cmd "%%i\ssglog.properties"
...
for /F "tokens=1,* delims==" %%i in (ssg\tomcat\webapps\ROOT\WEB-INF\classes\WsdmSOMMA_Basic.properties) do if "%%i"=="log.file.path" (echo log.file.path=%SSG_HOME_ESC%\\logs\\ca_wsdm_observer>> WsdmSOMMA_Basic.properties.EDIT) else (echo %%i=%%j>> WsdmSOMMA_Basic.properties.EDIT)
...
xcopy /F /Y ssg\tomcat\webapps\ROOT\WEB-INF\lib\ca_wsdm_observer.jar "%SSG_HOME%\tomcat\webapps\ROOT\WEB-INF\lib\"
...
if exist "%SSG_HOME%\etc\conf\ca_wsdm_observer.properties" (
    echo The file "%SSG_HOME%\etc\conf\ca_wsdm_observer.properties" already exists. It will not be overwritten.
) else (
    xcopy /F /Y ssg\etc\conf\ca_wsdm_observer.properties "%SSG_HOME%\etc\conf\"
)
