...
public class NetlogonLogonIdentityInfo extends NdrObject{
     ...
     public void decode(NdrBuffer src) throws NdrException {
       src.align(4);
       src.align(4);
       if (logonDomainName == null) {
         logonDomainName = new rpc.unicode_string();
       }
       logonDomainName.length = (short)src.dec_ndr_short();
       logonDomainName.maximum_length = (short)src.dec_ndr_short();
       int logonDomainNameRef = src.dec_ndr_long();
       parameterControl = src.dec_ndr_long();
       logonIdLow = src.dec_ndr_long();
       logonIdHigh = src.dec_ndr_long();
       src.align(4);
       if (userName == null) {
         userName = new rpc.unicode_string();
       }
       userName.length = (short)src.dec_ndr_short();
       userName.maximum_length = (short)src.dec_ndr_short();
       int userNameRef = src.dec_ndr_long();
       src.align(4);
       if (workstation == null) {
         workstation = new rpc.unicode_string();
       }
       workstation.length = (short)src.dec_ndr_short();
       workstation.maximum_length = (short)src.dec_ndr_short();
       int workstationRef = src.dec_ndr_long();

       if (logonDomainNameRef != 0) {
         src = DceRpcUtil.decode_rpc_string(src, this.logonDomainName);
       }
       if (userNameRef != 0) {
         src = DceRpcUtil.decode_rpc_string(src, this.userName);
       }
       if (workstationRef != 0) {
         src = DceRpcUtil.decode_rpc_string(src, this.workstation);
       }
     }
}
