...
public class JDBCMigration extends com.l7tech.skunkworks.rest.tools.MigrationTestBase {
    ...
    @Test
    public void testIgnoreJdbcConnectionDependencies() throws Exception {
        RestResponse response = getSourceEnvironment().processRequest("bundle?jdbcConnection=" + jdbcConnectionItem.getId() + "&requireJdbcConnection=" + jdbcConnectionItem.getId(), HttpMethod.GET, null, "");
        logger.log(Level.INFO, response.toString());
        assertOkResponse(response);

        Item<Bundle> bundleItem = MarshallingUtils.unmarshal(Item.class, new StreamSource(new StringReader(response.getBody())));

        assertEquals("The bundle should have 1 items. A jdbcConnection", 1, bundleItem.getContent().getReferences().size());
        assertEquals("The bundle should have 1 mapping. A jdbcConnection", 1, bundleItem.getContent().getMappings().size());
