...
public class SampleMessageMigration extends com.l7tech.skunkworks.rest.tools.MigrationTestBase {
    private static final Logger logger = Logger.getLogger(SampleMessageMigration.class.getName());
    ...
    @Test
    public void testIgnoreSampleMessageDependencies() throws Exception {
        Item<SampleMessageMO> sourceSampleMessageItem = createSampleMessage(getSourceEnvironment(), "MyMessage");

        RestResponse response = getSourceEnvironment().processRequest("bundle?sampleMessage=" + sourceSampleMessageItem.getId() + "&requireSampleMessage=" + sourceSampleMessageItem.getId(), HttpMethod.GET, null, "");
        logger.log(Level.INFO, response.toString());
        assertOkResponse(response);

        Item<Bundle> bundleItem = MarshallingUtils.unmarshal(Item.class, new StreamSource(new StringReader(response.getBody())));

        assertEquals("The bundle should have 1 items. A sampleMessage", 1, bundleItem.getContent().getReferences().size());
        assertEquals("The bundle should have 1 mapping. A sampleMessage", 1, bundleItem.getContent().getMappings().size());
