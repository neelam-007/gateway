...
public class CustomKeyValueMigration extends com.l7tech.skunkworks.rest.tools.MigrationTestBase {
    ...
    @Test
    public void testIgnoreCustomKeyValueDependencies() throws Exception {
        RestResponse response = getSourceEnvironment().processRequest("bundle?customKeyValue=" + customKeyValueStoreItem.getId() + "&requireCustomKeyValue=" + customKeyValueStoreItem.getId(), HttpMethod.GET, null, "");
        logger.log(Level.INFO, response.toString());
        assertOkResponse(response);
        ...
        assertEquals("The bundle should have 1 items. A customKeyValue", 1, bundleItem.getContent().getReferences().size());
        assertEquals("The bundle should have 1 mapping. A customKeyValue", 1, bundleItem.getContent().getMappings().size());
