...
public class PrivateKeyRestEntityResourceTest extends RestEntityTests<SsgKeyEntry, PrivateKeyMO> {
    ...
    @Override
    public List<PrivateKeyMO> getUpdateableManagedObjects() throws Exception {
        ...
        certificateData.setIssuerName(URLEncoder.encode("CN=myalias", "UTF-8"));
        certificateData.setSubjectName(URLEncoder.encode("CN=testkey", "UTF-8"));
        ...
        certificateData2.setIssuerName(URLEncoder.encode("CN=myalias", "UTF-8"));
        certificateData2.setSubjectName(URLEncoder.encode("CN=myalias", "UTF-8"));
    ...
    public void verifyEntity(String id, PrivateKeyMO managedObject) throws Exception {
        ...
        if (managedObject == null) {
            ...
            for (int i = 0; i < entity.getCertificateChain().length; i++) {
                ...
                Assert.assertEquals(URLEncoder.encode(cert.getSubjectDN().getName().replaceAll(", ", ","), "UTF-8"), certificateData.getSubjectName());
                Assert.assertEquals(URLEncoder.encode(cert.getIssuerDN().getName().replaceAll(", ", ","), "UTF-8"), certificateData.getIssuerName());
