...
public class GenericEntityMigration extends com.l7tech.skunkworks.rest.tools.MigrationTestBase {
    ...
    @Test
    public void testIgnoreGenericEntityDependencies() throws Exception {
        RestResponse response = getSourceEnvironment().processRequest("bundle?genericEntity=" + genericEntityItem.getId() + "&requireGenericEntity=" + genericEntityItem.getId(), HttpMethod.GET, null, "");
        logger.log(Level.INFO, response.toString());
        assertOkResponse(response);

        Item<Bundle> bundleItem = MarshallingUtils.unmarshal(Item.class, new StreamSource(new StringReader(response.getBody())));

        assertEquals("The bundle should have 1 items. A genericEntity", 1, bundleItem.getContent().getReferences().size());
        assertEquals("The bundle should have 1 mapping. A genericEntity", 1, bundleItem.getContent().getMappings().size());
