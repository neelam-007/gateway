...
public class HttpConfigurationMigration extends com.l7tech.skunkworks.rest.tools.MigrationTestBase {
    ...
    @Test
    public void testIgnoreHttpConfigurationDependencies() throws Exception {
        RestResponse response = getSourceEnvironment().processRequest("bundle?httpConfiguration=" + httpConfigurationItem.getId() + "&requireHttpConfiguration=" + httpConfigurationItem.getId(), HttpMethod.GET, null, "");
        logger.log(Level.INFO, response.toString());
        assertOkResponse(response);
        ...
        assertEquals("The bundle should have 1 items. A httpConfiguration", 1, bundleItem.getContent().getReferences().size());
        assertEquals("The bundle should have 1 mapping. A httpConfiguration", 1, bundleItem.getContent().getMappings().size());
