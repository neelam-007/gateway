 ...
 */
public class SimpleOtherSolutionKitManagerCallback extends BaseSolutionKitManagerCallback {
    ...
    private static final String SERVICE_GUID = "33b16742-d62d-4095-8f8d-4db707e9ad55";

    @Override
    protected void OnPreMigrationBundleImport(SolutionKitManagerContext context) throws CallbackException {
        // read bundle
        ...
        final String message = "*** CUSTOM CODE CALLED FOR " + getSolutionKitName() + " ***  # item(s) in bundle: " + itemElements.size() + ", # mapping(s): " + mappingElements.size() + ", instance modifier: " + context.getInstanceModifier() + ". " + input;
        logger.info(message);
        System.out.println(message);

        // force user to always provide input
        if (input == null) {
            String requireInput = "Solution kit '" + getSolutionKitName() + "' requires additional user input. ";
        ...
        if (!beenCustomized) {
            ...
            if (nameElements.size() > 0) {
                nameElements.get(0).setTextContent(input + " " + getSolutionKitName());
            }

            // create a new folder
            createFolder(restmanBundle, context.getUninstallBundle(), isUpgrade(), getSolutionKitVersion().equals(getInstalledSolutionKitVersion()), input);
        ...
        // print the keyValues for the service kit
        printOutContextMap(SERVICE_GUID);
    }
