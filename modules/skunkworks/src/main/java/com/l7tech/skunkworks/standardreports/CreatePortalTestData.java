 ...
 *
 * Hacked version of CreateTestData for creating Portal API data.
 *
 * Modify as needed. CreateTestData was written for a different set of data than
 * required by the Portal (portal requires 2 or more CUSTOM_MAPPINGs).
 * Hacked this version as something portal can work with for sample data.
 */
...
public class CreatePortalTestData {
    ...
    private List<Long> mappingKeyObjectIdList =  new ArrayList<Long>();
    private final LinkedHashMap<String,List<String>> values;
    private final List<String> operations;
    private final List<String> authenticatedUser;
    private List<List<Map<Long, String>>> valueInsertsList = new ArrayList<List<Map<Long, String>>>();
     ...
     * @param operations
     */
    public CreatePortalTestData(File outputDir,
                                long objectId,
                                String nodeId,
                                List<LinkedHashMap<String, String>> mappingKeys,
    ...
    private void initMappingRecords(){
        for(LinkedHashMap<String, String> currentKeyMap: mappingKeys){
            ...
            sb.append(", create_time) VALUES (" + objectId + ",1,\"NOT USED\"");

            for(String s: currentKeyMap.keySet()){
                if (s.startsWith("CUSTOM_MAPPING")) {
                    sb.append(",\"CUSTOM_MAPPING\",");
                } else {
                    sb.append(",\"" + s + "\",");
                }
...
*/
        ...
        for (int i1 = 0; i1 < mappingKeys.size(); i1++) {
            ...
            List<Map<Long, String>> currentValueListMap = new ArrayList<Map<Long, String>>();
            objectId++;
            ...
            for (String s : currentKeyMap.values()) {
            ...
            for (List<String> list : completeList) {
                ...
                long newValueKey = objectId++;
                sb.append(newValueKey + ",\"NOT USED\", " + mappingKeyObjectIdList.get(i1));
                sb.append(",NULL");//auth user
                sb.append(",NULL");//operation
...
//                }
                ...
                map.put(new Long(newValueKey), sb.toString());
...
//            initialSize += newLists.size();
    ...
    public void createMappingData() throws Exception{
        ...
        try{
            File f = new File(outputDir, "MappingRecords.sql");
            ...
            for(List<Map<Long, String>> listMap: valueInsertsList){
    ...
    public void createMetricData(Utilities.UNIT_OF_TIME unitOfTime, String serviceId) throws Exception{
        ...
        try{
            File f = new File(outputDir, fileName + "_" + serviceId + ".sql");
            if(!f.exists()) f.createNewFile();
            else{
                f.delete();
                f.createNewFile();
...
*/
        ...
        for(List<Map<Long, String>> currentValueListMap: valueInsertsList){
            for(Map<Long, String> map: currentValueListMap){
...
//        System.out.println(seed + " " + numAttempted + " " + numAuthorized+ " " + numCompleted);
    ...
    public static void main(String [] args) throws Exception {
        ...
        args = new String[]{"800000",
                "/home/darmstrong/ideaprojects/UneasyRooster/modules/skunkworks/src/main/resources/com/l7tech/skunkworks/standardreports/test/portaldata.properties",
                "/home/darmstrong/Documents/Projects/Portal/TestData"};

        if (args.length < 3) {
            System.out.println("Usage: CreateTestData startobjectid propertiesfile outputfolder");
            System.exit(1);
        }

        long objectId = Long.valueOf(args[0]);
        ...
        LinkedHashMap<String, String> nameToId = loadMapFromProperties("SERVICE_ID_TO_NAME", "SERVICE_ID_TO_NAME_OID", prop);
        ...
        if (mappingKeyList.isEmpty())
            throw new IllegalArgumentException("At least one key mapping row must be defined");
        ...
        for (String mappingType : mappingKeyFirstMap.values()) {
            List<String> mappingValueList = loadListFromProperties(mappingType, prop);
            values.put(mappingType, mappingValueList);
        }

        List<String> operations = Collections.emptyList();
        List<String> authenticatedUser = Collections.emptyList();

        String nodeId = prop.getProperty(NODE_ID);
        CreatePortalTestData testData = new CreatePortalTestData(outputFolder, objectId, nodeId, mappingKeyList, values, operations, authenticatedUser);
        ...
        for (String s : nameToId.values()) {
            ...
            System.out.println("Hourly & Daily data created for service: " + s);
        }
    ...
    public static LinkedHashMap<String, String> loadMapFromProperties(String key1, String key2, Properties prop) {
        ...
        while (key1Name != null && key2Name != null) {
            if (key1Name.equals("CUSTOM_MAPPING")) {
                returnMap.put(key2Name, key2Name);
            } else {
