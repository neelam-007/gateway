...
public class Xss4jWrapper {
    ...
    private boolean decryptElement(Document soapMsg, String elementName, Key key) throws Exception {
        ...
        Element encryptedDataElement = DomUtils.findFirstChildElementByName(messagePartElement,
                                                                           SoapUtil.XMLENC_NS,
                                                                           "EncryptedData");
        ...
        Element header = DomUtils.findFirstChildElementByName(envelope, envelopeNs, "Header");
        if (header == null)
            throw new IllegalArgumentException("Encrypted data is present, but there is no SOAP header");

        Element security = DomUtils.findFirstChildElementByName(header, SoapUtil.SECURITY_URIS_ARRAY, "Security");
        if (security == null)
            throw new IllegalArgumentException("Encrypted data is present, but there is no security element");

        List referenceListList = DomUtils.findChildElementsByName(security, SoapUtil.XMLENC_NS, SoapUtil.REFLIST_EL_NAME);
        ...
        for (Iterator i = referenceListList.iterator(); i.hasNext();) {
            ...
            List dataRefEls = DomUtils.findChildElementsByName(referenceList, SoapUtil.XMLENC_NS, SoapUtil.DATAREF_EL_NAME);
