        ...
        implements ServiceManager
{
    ...
    public void addManageServiceRole(PublishedService service) throws SaveException {
        ...
        // RUD this service
        newRole.addPermission(OperationType.READ, SERVICE, service.getId()); // Read this service
        newRole.addPermission(OperationType.UPDATE, SERVICE, service.getId()); // Update this service
        newRole.addPermission(OperationType.DELETE, SERVICE, service.getId()); // Delete this service

        // Read information generated by messages sent to this service
        newRole.addPermission(OperationType.READ, CLUSTER_INFO, null); // Prerequisite to viewing audits and metrics
        newRole.addPermission(OperationType.READ, METRICS_BIN, MetricsBin.ATTR_SERVICE_OID, service.getId());
        newRole.addPermission(OperationType.READ, AUDIT_MESSAGE, MessageSummaryAuditRecord.ATTR_SERVICE_OID, service.getId());
        newRole.addPermission(OperationType.READ, SERVICE_USAGE, ServiceUsage.ATTR_SERVICE_OID, service.getId());

        // Search identities (for adding IdentityAssertions)
        newRole.addPermission(OperationType.READ, ID_PROVIDER_CONFIG, null);
        newRole.addPermission(OperationType.READ, USER, null);
        newRole.addPermission(OperationType.READ, GROUP, null);

        // Read JMS queues
        newRole.addPermission(OperationType.READ, JMS_CONNECTION, null);
        newRole.addPermission(OperationType.READ, JMS_ENDPOINT, null);
        ...
        if (currentUser != null) {
            ...
            boolean omnipotent;
            try {
                omnipotent = roleManager.isPermittedForAllEntities(currentUser, SERVICE, OperationType.READ);
                omnipotent &= roleManager.isPermittedForAllEntities(currentUser, SERVICE, OperationType.UPDATE);
                omnipotent &= roleManager.isPermittedForAllEntities(currentUser, SERVICE, OperationType.DELETE);
                omnipotent &= roleManager.isPermittedForAllEntities(currentUser, METRICS_BIN, OperationType.READ);
                omnipotent &= roleManager.isPermittedForAllEntities(currentUser, AUDIT_MESSAGE, OperationType.READ);
                omnipotent &= roleManager.isPermittedForAllEntities(currentUser, ID_PROVIDER_CONFIG, OperationType.READ);
                ...
                omnipotent &= roleManager.isPermittedForAllEntities(currentUser, CLUSTER_INFO, OperationType.READ);
                omnipotent &= roleManager.isPermittedForAllEntities(currentUser, SERVICE_USAGE, OperationType.READ);
                omnipotent &= roleManager.isPermittedForAllEntities(currentUser, JMS_CONNECTION, OperationType.READ);
                omnipotent &= roleManager.isPermittedForAllEntities(currentUser, JMS_ENDPOINT, OperationType.READ);
            } catch (FindException e) {
