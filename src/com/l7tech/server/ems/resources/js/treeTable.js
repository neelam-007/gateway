...
function l7_treeTable(table) {
    ...
    function toggle() {
        ...
        function isExpanded(toggler) {
            return getFileName(toggler.src) == EXPANDED_TOGGLER_IMG_NAME;
        }
        ...
        function setExpanded(toggler, expanded) {
            toggler.src = getParent(toggler.src) + (expanded ? EXPANDED_TOGGLER_IMG_NAME : COLLAPSED_TOGGLER_IMG_NAME);
        }
        ...
        function hideDescendents() {
            ...
            for (var i = rowIndex + 1; i < rows.length; ++i) {
                ...
                if (parentId != null) {
                    if (arrayContains(rowsToHideParentIds, parentId)) {
        ...
        function unhideDescendents() {
            ...
            for (var i = rowIndex + 1; i < rows.length; ++i) {
                ...
                var parentId = parentIds[i];
                if (parentId != null) {
                    if (arrayContains(rowsToUnhideParentIds, parentId)) {
                        row.style.display = '';
    ...
    function check() {
        ...
        function getState(checkbox) {
            if (checkbox.checked) {
                if (hasClass(checkbox, MIXED_STYLE_CLASS)) {
                    return STATE_MIXED;
                } else {
         ...
         *
         * @param checkbox      the checkbox INPUT element
         * @param state         STATE_UNCHECKED, STATE_CHECKED or STATE_MIXED
         */
        function setState(checkbox, state) {
            if (state == STATE_CHECKED) {
                checkbox.checked = true;
                removeClass(checkbox, MIXED_STYLE_CLASS);
            } else if (state == STATE_UNCHECKED) {
                checkbox.checked = false;
                removeClass(checkbox, MIXED_STYLE_CLASS);
            } else if (state == STATE_MIXED) {
                checkbox.checked = true;
                addClass(checkbox, MIXED_STYLE_CLASS);
            }
        }
        ...
        function checkDescendents(checked) {
            ...
            for (var i = rowIndex + 1; i < rows.length; ++i) {
                ...
                var parentId = parentIds[i];
                if (parentId != null) {
                    if (arrayContains(rowsToCheckParentIds, parentId)) {
        ...
        removeClass(this, MIXED_STYLE_CLASS);
    ...
    this.init = function() {
        ...
        for (var i = 0; i < rows.length; ++i) {
            ...
            // Looks for parent ID.
            var parentId = null;
            var tds = getElementsByClassName('parentId', row, 'td');
            if (tds.length == 1) {
